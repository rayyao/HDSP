...
from lib.train.dataset.HSItrain import HSI
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        "DepthTrack_train", "DepthTrack_val", "LasHeR_all", "LasHeR_train", "LasHeR_val", "VisEvent","HSItrain"]
        if name == "HSItrain":
            datasets.append(HSI(settings.env.HSItrain_dir))
...
def build_dataloaders(cfg, settings):
    ...
    data_processing_train = processing.HDSPProcessing(search_area_factor=search_area_factor,
                                                       output_sz=output_sz,
    ...
    data_processing_val = processing.HDSPProcessing(search_area_factor=search_area_factor,
                                                     output_sz=output_sz,
                                                     center_jitter_factor=settings.center_jitter_factor,
                                                     scale_jitter_factor=settings.scale_jitter_factor,
                                                     mode='sequence',
