...
from viptnew.lib.train.trainers import BaseTrainer
from viptnew.lib.train.admin import AverageMeter, StatValue
from viptnew.lib.train.admin import TensorboardWriter
import torch
import time
import torch
...
from viptnew.lib.utils.misc import get_world_size
import numpy as np
import torch
...
class autocast:
    ...
    def __call__(self, func):
        def wrapper(*args, **kwargs):
            args = [torch.cuda.amp.autocast()(arg) if isinstance(arg, torch.Tensor) else arg for arg in args]
            kwargs = {k: torch.cuda.amp.autocast()(v) if isinstance(v, torch.Tensor) else v for k, v in kwargs.items()}
