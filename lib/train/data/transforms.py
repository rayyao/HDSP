...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        if dims[0] == 9:
            ...
            depth1 = tvisf.normalize(depth1, self.mean, self.std, self.inplace)
            depth2 = tvisf.normalize(depth2, self.mean, self.std, self.inplace)
            return torch.cat((rgb, depth1,depth2), 0)
        else:
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            image2=image[:,:,3:6]
            image3=image[:, :,6:9]
            img_gray1 = cv.cvtColor(image1, cv.COLOR_RGB2GRAY)
            img_gray2 = cv.cvtColor(image2, cv.COLOR_RGB2GRAY)
            img_gray3 = cv.cvtColor(image3, cv.COLOR_RGB2GRAY)
            n1=np.stack([img_gray1, img_gray1, img_gray1], axis=2)
            n2 = np.stack([img_gray2, img_gray2, img_gray2], axis=2)
            n3 = np.stack([img_gray3, img_gray3, img_gray3], axis=2)
            n=np.concatenate((n1,n2,n3),axis=2)
            return n
